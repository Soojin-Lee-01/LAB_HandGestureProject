...
extract_path = '../NewData/voxels/Train_'
...
#sub_dirs=['Circle', 'DiagonalLeftToRight', 'DiagonalRightToLeft', 'FistForward', 'FistSpread', 'LeftToRight']
...
for i in range(10):   # GPU 메모리 부족하여 dataset 나눠서 전체데이터에 대해 10번 반복. 전체 데데이터를 학습한다는 관점에서 일반적인 학습에서 epoch 수로 사용
    print('loop ', i+1, '-------------------')
    for index in range(loop_cnt + 1): # 학습할 전체 dataset를 training_sample_cnt만큼 나누어 학습 진행
        ...
        if os.path.exists('test_model.h5'):   # 전이학습을 위해 저정된 모델이 있으면 읽어와서 weight update
    ...
    model.save('test_model'+str(i+1)+'.h5')
