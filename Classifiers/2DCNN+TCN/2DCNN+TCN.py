...
extract_path = '../../Data/Train_' # projection 데이터 경로(Train_은 건들면 안됨)

# if time_steps > tcn_layer.receptive_field, then we should not
# be able to solve this task.
img_size = 32 # projection 이미지 크기 (현재 32* 32)
time_steps, input_dim = 27, img_size*img_size*1
# time_steps는 시퀀스 길이(현재 27프레임으로 설정)
# input_dim는 img_size에 따라 자동 설정되므로 수정할 필요 X

# 제스처 클래스
sub_dirs=['Spread']

# 모델 훈련 관련 설정
checkpoint_model_path= "../../models/" # 체크포인트 저장 경로
EPOCHS = 30 # 에폭 수
BATCH_SIZE = 16 # 배치 사이즈
MODEL_SAVE_PATH = 'model.h5' # 모델 파일 저장 경로
...
def full_model(input_x, input_y):
    ...
    model = Sequential()
    model.add(TimeDistributed((Conv2D(16, (3, 3), strides=(1, 1), input_shape=(img_size, img_size,1), padding="same", activation="relu", name="Conv2D_1"))))
    model.add(TimeDistributed(MaxPooling2D(strides=(3, 3), pool_size=(3, 3), padding="valid", name="MaxPool_1")))

    model.add(TimeDistributed((Conv2D(32, (3, 3), strides=(1, 1), padding="same", activation="relu", name="Conv2D_2"))))
